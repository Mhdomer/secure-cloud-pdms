...
| Sprint 5 | Security evaluation: scans, RTO test, Security Hub, UAT | **In progress** — coverage check, Checkov (399/0/46), Trivy (filesystem + a real CI-built container image, 0 vulnerabilities both), and a real CI-triggered SonarQube run (clean on security/reliability/maintainability/duplication/hotspots; fails only on new-code test coverage, since this repo has no automated test suite yet) all done; white-box + local black-box pentest found and fixed 1 CRITICAL cross-tenant IDOR (`sick_leaves` had no RLS) plus 4 smaller findings, CRITICAL fix live-verified against real fixture data; HIPAA §164.312 posture assessed against the IaC design (a proxy, not a live score); **RTO test executed live 2026-07-31 — RTO = 47m48s, misses the ≤15 min NFR-06 target** (RDS Multi-AZ alone takes ~15 min; a human-gated app-rollout step is structurally unavoidable regardless of automation); **AWS Security Hub attempted, blocked by an account-level `SubscriptionRequiredException`** (not fixable from Terraform/CLI — needs an AWS Support case) — see `docs/psm2/sprints/sprint-5-security-evaluation.md` §6–7. Infra was redeployed live then fully torn down again afterward (independently re-verified empty) — back to the zero-cost state. Not done: UAT with 3+ human participants (`docs/psm2/sprints/sprint-5-uat-plan.md`), the only Sprint 5 deliverable still fully unexecuted, pending human coordination (no AWS cost) |
