...
| Sprint 5 | Security evaluation: scans, RTO test, Security Hub, UAT | **In progress** — coverage check, Checkov (399/0/46), Trivy (filesystem + a real CI-built container image, 0 vulnerabilities both), and a real CI-triggered SonarQube run (clean on security/reliability/maintainability/duplication/hotspots; fails only on new-code test coverage, since this repo has no automated test suite yet) all done; white-box + local black-box pentest found and fixed 1 CRITICAL cross-tenant IDOR (`sick_leaves` had no RLS) plus 4 smaller findings, CRITICAL fix live-verified against real fixture data; HIPAA §164.312 posture assessed against the IaC design (a proxy, not a live score); **RTO test executed live 2026-07-31 — RTO = 47m48s** (RDS Multi-AZ alone takes ~15 min; a human-gated app-rollout step is structurally unavoidable regardless of automation). The ≤15 min NFR-06 target this missed was **resolved 2026-09-16** by splitting it into NFR-06a (≤25 min infrastructure) and NFR-06b (≤60 min full service), both of which the drill meets — see `docs/psm2/nfr-06-rto-decision-2026-09-15.md` and Appendix D Table D.5; **AWS Security Hub attempted, blocked by an account-level `SubscriptionRequiredException`** (not fixable from Terraform/CLI — needs an AWS Support case) — see `docs/psm2/sprints/sprint-5-security-evaluation.md` §6–7. Infra was redeployed live then fully torn down again afterward (independently re-verified empty) — back to the zero-cost state. Not done: UAT with 3+ human participants (`docs/psm2/sprints/sprint-5-uat-plan.md`), the only Sprint 5 deliverable still fully unexecuted, pending human coordination (no AWS cost) |
...
- RTO targets: **NFR-06a ≤ 25 min** (infrastructure, unattended — measured 17m40s) and **NFR-06b ≤ 60 min** (full service incl. the deploy approval gate — measured 47m48s). Revised 2026-09-16 from a single ≤ 15 min target that was set without measurement and is unreachable: RDS Multi-AZ provisioning alone is ~15 min, and the `production` required-reviewer gate makes unattended recovery impossible by design. See `docs/psm2/nfr-06-rto-decision-2026-09-15.md`
...
- NFRs (including RTO — now NFR-06a ≤25min / NFR-06b ≤60min — and 99.9% uptime): Chapter 3 Section 3.5.2, full table in `docs/report/appendices.md` Table D.2 (as submitted) and Table D.5 (Sprint 5 revision)
...
- Sprint 5 security evaluation (2026-07-31) — scan results, HIPAA §164.312 IaC-level posture, pentest findings/fixes/live-verification, grill-me score, live RTO test result (47m48s, misses target) with root-cause breakdown, and the Security Hub account-eligibility block: `docs/psm2/sprints/sprint-5-security-evaluation.md` (local-only, gitignored — not on GitHub). Ready-to-execute (not yet run) plans: `docs/psm2/sprints/sprint-5-uat-plan.md` (UAT, still pending) and `sprint-5-security-hub-plan.md` (blocked on AWS account eligibility, not this session); `sprint-5-rto-test-plan.md`'s plan has now been executed against — see the evaluation doc §6
- **en/ar "key parity" was being enforced wrongly** (2026-09-13) — `/i18n-check` compared *raw* keys, so Arabic's `zero/two/few/many` plural forms read as 12 keys "missing from English." A previous session silenced it by padding `en/patients.json` and `en/settings.json` with those categories; all 12 were unreachable (`Intl.PluralRules('en')` resolves only `one`/`other`) and three hardcoded the number instead of `{{count}}`. Padding reverted, the gate rewritten to compare base keys and validate plural categories per language, and the check made executable rather than judgment-based: `npm run i18n:check` in `src/frontend` (`scripts/check-i18n-parity.mjs`), negative-tested against both a dead `en` category and a dropped `ar` one.
- **CI/CD deploy pipeline was never actually deploying anything** (2026-08-05) — every push to `main` showed a red X because SonarQube's coverage gate was judging *every commit since the project's first-ever scan* (2026-07-27) instead of just each PR's diff, an unwinnable bar with no test suite. Fixed with a real backend Jest suite + the actual root cause (`sonar.projectVersion` was never set). Confirmed passing on a real CI run. Full root-cause writeup, the "one-analysis lag" gotcha, and what's still not covered: `docs/psm2/sonarqube-coverage-gate-fix-2026-08-05.md`
- **`main` still shows a red X, and that is the designed behaviour — do not "fix" it** (clarified 2026-09-15). Two separate things get conflated here:
  - **Security gates pass.** SAST/SonarQube, the Trivy container scan, and Checkov all run on every push and go green. That is the part the 2026-08-05 fix was about.
  - **Deploy is held at a human approval gate.** All three deploy jobs declare `environment: production` (`deploy.yml:43`, `:98`, `:233`), and that environment carries a required-reviewer rule. With AWS infra deliberately torn down to a zero-cost state, nobody approves, so those jobs sit pending and GitHub renders the run as failed. The red X is a control working, not a regression — treat a *green* deploy job while infra is torn down as the thing worth investigating.
- **Backend test suite now covers the security controls, not just utilities** (2026-09-15) — 192 tests. JWT verification (real signatures, tampering, `alg=none`, cookie-only delivery), the full RBAC role matrix, the RLS session bootstrap, the `withTransaction` session-GUC contract, and a live-PostgreSQL cross-tenant isolation suite that is the regression test for the Sprint 5 `sick_leaves` IDOR. The live suite seeds and asserts inside one rolled-back transaction, so it leaves the dev DB byte-identical and needs no privileged credentials. **It is skipped in CI** (`SKIP_DB_TESTS=1` in `security-scan.yml`) because the runner has no PostgreSQL — so the database half of the two-layer RBAC claim is verified locally only. Closing that needs a postgres service container plus a seed step; see the comment on that workflow step.
- **RLS extended to `appointments` and `patient_invoices`** (2026-09-15) — both held patient data behind application-layer ownership checks alone. The trap worth knowing: self-booking's double-booking check runs inside the *patient's* own session, so once `appointments` was RLS-protected a direct conflict query saw an empty table and would have let two patients book the same slot. Conflict detection now goes through the `appointment_conflict_id()` / `doctor_slot_taken()` SECURITY DEFINER helpers, which return only an id and a boolean — a patient learns a slot is taken, never whose it is. Full detail: `docs/psm2/rls-expansion-2026-09-15.md`
- **A committed local Postgres superuser password** was found in `docs/psm2/session-prompts/backend-schema-gaps.md` (commit `3d00815`, public). Localhost-only, so low practical severity, but it crosses this file's own "never commit credentials" gate. Redacted going forward; history deliberately not rewritten; **rotation is still outstanding and is a manual step** — `docs/psm2/credential-exposure-2026-09-15.md`
