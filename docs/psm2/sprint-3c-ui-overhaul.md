...
| 18 | Forgot Password Page (new, public) | `src/pages/auth/ForgotPasswordPage.tsx` | Done (DELTA-047 in `report-delta.md`) — 2-step patient self-service phone-OTP password reset, reusing UC-19's OTP/token infrastructure end-to-end (schema, models, `generateSetupToken`, rate limiter) rather than a parallel mechanism. Staff accounts see a static "contact your administrator" notice instead of a form — no verified contact channel exists for them. New "Forgot password?" link added under the password field on `LoginPage.tsx` (Screen 1) as the entry point; `SetupPasswordPage.tsx` (Screen 15) reused unmodified as the flow's final step. Full design rationale: `docs/superpowers/specs/2026-07-24-forgot-password-design.md`. Verified live end-to-end (happy path, non-enumeration, rate limiting, unlock-on-reset via real HTTP calls against the local dev DB); formal `/rtl-check` + `/ui-review` not yet run. | 2026-07-25 |
