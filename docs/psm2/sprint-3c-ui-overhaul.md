...
| 2 | Doctor Dashboard | `src/pages/dashboard/DoctorDashboard.tsx` | Gate passed — follow-up session added the split Consultation/Treatment Room hero banner, a 5th "Follow-Ups" stat card, and a "Next Patient" summary card (avatar/MRN/age/blood type/allergies/last visit, bound to the day's next appointment) in the right column above Recent Patients. See DELTA-022 in `report-delta.md`. Further rebuilt for the walk-in billing flow — "Today's Queue" promoted to the top of the page as an interactive consultation/billing card (add priced services, prescription notes, mark patient done). See DELTA-026/DELTA-027 in `report-delta.md` | 2026-07-14, 2026-07-18 |
| 3 | Staff Dashboard | `src/pages/dashboard/AdminDashboard.tsx` | Gate passed — follow-up session added the lobby hero banner + numbered Queue/Checked-In/In-Consultation flow pills, and replaced the vertical appointment timeline with a proper Time/Patient/Type/Doctor/Status table (matching the Canva reference); a "Quick Actions" list was deliberately NOT added — it would have duplicated the two existing bold action tiles. See DELTA-022 in `report-delta.md` | 2026-07-15, 2026-07-18 |
| 4 | Patient Dashboard | `src/pages/dashboard/PatientDashboard.tsx` | Gate passed — follow-up session added the lobby hero banner with a frosted "Welcome back" card, split the single appointment card into separate "Upcoming Appointment" (info-only) and "Quick Book" (always-on CTA) cards, and added a decorative "Your Care Journey" 4-step stepper. See DELTA-022 in `report-delta.md` | 2026-07-15, 2026-07-18 |
...
| 13 | Landing Page | `src/pages/landing/LandingPage.tsx` | Gate passed — follow-up session 2026-07-18 added a new "Our Specialty Centres" interactive section (5 specialties, left list + right pre-designed image card with crossfade + visible next-item preview), redesigned `/services` cards (icon + real clinic photo + title/desc, all 8 services now imaged), added a "Take a Look Inside" photo gallery to `/facilities`, and a photo accent to the `/patient-info` FAQ section. See DELTA-021 in `report-delta.md` | 2026-07-17, 2026-07-18 |
